...
-(void)selectDate:(long int)date {
   ...
   long int selectedDateYear = [selectedDate year];
   long int selectedDateMonth = [selectedDate month];
   long int currentMonthYear = [currentMonth year];
   long int currentMonthMonth = [currentMonth month];
...
-(void)showNextMonth {
    ...
    long int lastBlock = [currentMonth firstWeekDayInMonth]+[currentMonth numDaysInMonth]-1;
    long int numBlocks = [self numRows]*7;
...
{       
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight) {
        ...
        long int column = floorf(xLocation/(kVRGCalendarViewDayWidth+2));
        int row = floorf(yLocation/(kVRGCalendarViewDayHeight+2));
        
        long int blockNr = (column+1)+row*7;
        long int firstWeekDay = [self.currentMonth firstWeekDayInMonth]-1; //-1 because weekdays begin at 1, not 0
        long int date = blockNr-firstWeekDay;
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
       ...
       long int currentMonthIndex = [self.currentMonth month];
       long int todayMonth = [[NSDate date] month];
        [self reset];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGRect rectangle = CGRectMake(0,0,kVRGCalendarViewWidth,kVRGCalendarViewTopBarHeight);
    ...
    long int numBlocks = numRows*7;
    NSDate *previousMonth = [self.currentMonth offsetMonth:-1];
    long int currentMonthNumDays = [currentMonth numDaysInMonth];
    long int prevMonthNumDays = [previousMonth numDaysInMonth];
    
    long int selectedDateBlock = ([selectedDate day]-1)+firstWeekDay;
    ...
    if (isSelectedDatePreviousMonth) {
        long int lastPositionPreviousMonth = firstWeekDay-1;
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        long int targetBlock = firstWeekDay + (targetDate-1);
        long int targetColumn = targetBlock%7;
        long int targetRow = targetBlock/7;
        
        long int targetX = targetColumn * (kVRGCalendarViewDayWidth+2) + 7;
        long int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 38;
...
-(id)init {
    ...
    if (self) {
        ...
        NSLog(@"labelCurrentMonthlabelCurrentMonthlabelCurrentMonthlabelCurrentMonthlabelCurrentMonth1");
